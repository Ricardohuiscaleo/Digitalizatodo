            ...
            {today.length === 0 ? (
                ...
                <div className="space-y-2">
                    {today.map((s: any) => {
                        ...
                        // Cálculo de luminosidad para texto contrastado
                        const lum = bg !== '#f4f4f5' ? (() => { 
                        ...
                        return (
                            <div key={s.id} className="flex items-center gap-3 px-4 py-3 rounded-2xl transition-all" style={{ backgroundColor: bg, color: fg }}>
                                ...
                                <span className="text-sm font-black uppercase tracking-tight truncate">{s.subject || s.name || 'Clase'}</span>
                            </div>
                        );
                    })}
                </div>
