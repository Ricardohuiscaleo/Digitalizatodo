...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  position = 'right',
  colors = ['#FDBA74', '#B19EEF', '#5227FF'], // Added orange layer at start
  items = [],
  socialItems = [],
  displaySocials = true,
  ...
  const buildOpenTimeline = useCallback(() => {
    ...
    layerStates.forEach((ls, i) => {
      tl.fromTo(ls.el, { xPercent: ls.start }, { xPercent: 0, duration: 0.5, ease: 'power4.out' }, i * 0.05);
    });
  ...
  const toggleMenu = useCallback(() => {
    ...
    // We don't setOpen(target) immediately here, 
    // it's handled by GSAP callbacks for blur optimization.
    if (target) {
      onMenuOpen?.();
      playOpen();
    } else {
  ...
  return (
    ...
    >
      <div ref={preLayersRef} className="sm-prelayers" aria-hidden="true">
        {(() => {
          const raw = colors && colors.length ? colors.slice(0, 4) : ['#FDBA74', '#B19EEF', '#5227FF'];
          let arr = [...raw];
          if (arr.length >= 3) {
